...
type StatusDotColor = "gray" | "green" | "yellow" | "red";
...
export interface ServiceCardProps {
  ...
  onAction?: () => void;
  disabled?: boolean;
  // Slot bebas di header (kanan atas, sejajar badge) -- dipakai mis. switch
  // "MAVLink to QGC" di kartu MAVROS.
  headerExtra?: React.ReactNode;
  // Tombol tambahan di footer, di kiri tombol action utama (mis. Accept /
  // Cancel di kartu MISSION saat status running).
  extraButtons?: { label: string; onClick: () => void; disabled?: boolean }[];
  // Konten bebas antara details dan footer (mis. tombol "Detail" toggle).
...
const dotColorMap: Record<StatusDotColor, string> = {
  gray: "bg-slate-300",
  green: "bg-green-500",
  ...
  return (
    <Card className="h-full overflow-hidden border-slate-200 bg-white py-0 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-1 text-[9px] font-bold text-slate-400">
                <span className={`h-1.5 w-1.5 rounded-full ${dotColorMap[statusColor]}`} />
                {status}
              </div>
            </div>
          </div>
      ...
      <CardContent className="px-3 py-2.5">
        ...
        <div className="flex items-center justify-between gap-2">
          <span className="truncate font-mono text-[8px] text-slate-400">{footer}</span>
          <div className="flex items-center gap-1.5">
